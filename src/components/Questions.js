...
export default function Questions(props) {
  ...
  function concatenateChoices() {
    setQuiz((prevData) =>
      prevData.map((item, index) => {
        return {
          ...item,
          choices: insertRandom(item.incorrect_answers, item.correct_answer),
          id: index,
        };
      })
    );
  ...
  if (loading) {
    return (
      <div className="loading-wrapper">
        <img
          src="https://i.pinimg.com/originals/ae/1b/16/ae1b16df146614b08eca447625394346.gif"
          alt=""
        ></img>
      </div>
    );
  ...
  return (
    <div className="question-wrapper">
      {quizElements || "LoadingsyncAll()"}
      <div className="check_answer_btn-wrapper">
        {score.status && (
          <h1 className="score">
            You scored {score.score}/{quiz.length}
          </h1>
        )}
        <button
          className="check_answer_btn"
          onClick={!score.status ? checkAnswer : handleRestart}
        >
          {!score.status ? "Check Answer" : "Restart"}
        </button>
